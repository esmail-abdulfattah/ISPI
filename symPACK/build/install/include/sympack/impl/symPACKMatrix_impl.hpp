...
namespace symPACK{
  ...
  template<typename T>
void symPACKMatrix<T>::getfactorization_nonzero(std::vector<Int>& ia, std::vector<Int>& ja, std::vector<T>& a) {
    const T threshold = 1e-10;  // Adjust this threshold as necessary
    for (Int I = 1; I < this->Xsuper_.size(); I++) {
        Int src_first_col = this->Xsuper_[I - 1];
        Int iOwner = this->Mapping_->Map(I - 1, I - 1);
        if (iOwner == this->iam) {
            ...
            for (int blkidx = 0; blkidx < src_snode->NZBlockCnt(); ++blkidx) {
                ...
                for (Int i = 0; i < nRows; ++i) {
                    for (Int j = 0; j < src_snode->Size(); ++j) {
                        T value = val[i * src_snode->Size() + j];
                        if (std::abs(value) > threshold) {
                            ia.push_back(row + i);        // row index
                            ja.push_back(src_first_col + j);  // column index
                            a.push_back(value);          // non-zero value
                        }
                    }
                }
            }
...
template<typename T>
void symPACKMatrix<T>::getfactorization_full(std::vector<Int>& ia, std::vector<Int>& ja, std::vector<T>& a) {
    const T threshold = 1e-10;  // Adjust this threshold as necessary
    for (Int I = 1; I < this->Xsuper_.size(); I++) {
        Int src_first_col = this->Xsuper_[I - 1];
        Int iOwner = this->Mapping_->Map(I - 1, I - 1);
...
template<typename T>
T symPACKMatrix<T>::logDeterminant_ia_ja_a(std::vector<Int>& ia, std::vector<Int>& ja, std::vector<T>& a) {

    T logDet = 0.0;

    for (size_t i = 0; i < a.size(); i++) {
        if (ia[i] == ja[i]) { // this is a diagonal element
            if (a[i] <= 0) { // To handle negative and zero values
...
template<typename T>
T symPACKMatrix<T>::logDeterminant(bool fromsympack) {
    T logDet = 0.0;

    //std::cout << "size is: " << this->Xsuper_.size() << std::endl;
    for (Int I = 1; I < this->Xsuper_.size(); I++) {
        Int src_first_col = this->Xsuper_[I - 1];
        Int iOwner = this->Mapping_->Map(I - 1, I - 1);
        if (iOwner == this->iam) {
            ...
            for (int blkidx = 0; blkidx < src_snode->NZBlockCnt(); ++blkidx) {
                ...
                for (Int i = 0; i < nRows; ++i) {
                    for (Int j = 0; j < src_snode->Size(); ++j) {
                        if (row + i == src_first_col + j) { // this is a diagonal element
                            if (val[i * src_snode->Size() + j] <= 0) { // To handle negative and zero values
                                throw std::runtime_error("Matrix is not positive definite.");
                            }
                            logDet += std::log(val[i * src_snode->Size() + j]);
                            //std::cout << "log: " << std::log(val[i * src_snode->Size() + j]) << std::endl;
                        }
                    }
                }
            }
    ...
    if(fromsympack) return 2*logDet;
    else return logDet;
  ...
  template <typename T> inline void symPACKMatrix<T>::SymbolicFactorization(DistSparseMatrix<T> & pMat){
    ...
    //std::cout << "inside sympackmatrix_impl: Ordering is : " << this->options_.orderingStr << std::endl;

    //this->options_.orderingStr=="AMD";
          ...
          case METIS:
            {
              ...
              //std::cout << "after metis  "  << this->Order_.perm[0] << std::endl;
    ...
    else
    {
     ...
     // std::cout << "inside SymbolicFactorization: sgraph.rowind[0]:(1)  "  << this->Order_.perm[0] << std::endl;

      {
        double timeSta = get_time();
        //std::cout << "inside SymbolicFactorization: sgraph.rowind[0]:(2)  "  << this->Order_.perm[0] << std::endl;
        this->ETree_.ConstructETree(*sgraph,this->Order_,this->fullcomm_);
       // std::cout << "inside SymbolicFactorization: sgraph.rowind[0]:(3)  "  << this->Order_.perm[0] << std::endl;
        this->ETree_.PostOrderTree(this->Order_);
      //  std::cout << "inside SymbolicFactorization: sgraph.rowind[0]:(4)  "  << this->Order_.perm[0] << std::endl;
