...
// 4. Check memory usage (for Unix-like systems)
...
namespace symPACK{
  ...
  void Ordering::AMD(const SparseMatrixGraph & g, MPI_Comm comm){
    ...
    //std::cout << "stage AMD 1 " << std::endl;
    int iam =0;
    int np =1;
    MPI_Comm_rank(comm,&iam);
    MPI_Comm_size(comm,&np);

    //std::cout << "stage AMD 2 " << std::endl;
    ...
    if(iam==0){
      ...
      //std::cout << "stage AMD 10" << std::endl;
...
// 1. Print out the values of arguments
/*
std::cout << "N: " << N << std::endl;
std::cout << "IWLEN: " << IWLEN << std::endl;
std::cout << "PFREE: " << PFREE << std::endl;
std::cout << "NCMPA: " << NCMPA << std::endl;
std::cout << "IOVFLO: " << IOVFLO << std::endl;
*/

// 2. Check for null pointers
//if(!pcolptr || !prowind || !AMDperm || !AMDInvp || !&iwork[0]) {
 //   std::cerr << "Error: One or more pointers are null." << std::endl;
//    return; // or handle the error appropriately
//}

// 3. Check array sizes and contents
//std::cout << "First few elements of NVTXS: " << NVTXS[0] << ", " << NVTXS[1] << ", " << NVTXS[2] << std::endl;
//std::cout << "First few elements of QSIZE: " << QSIZE[0] << ", " << QSIZE[1] << ", " << QSIZE[2] << std::endl;
// ... add similar prints for other arrays if needed
...
// 5. Ensure array boundaries are not exceeded (assuming arrays are std::vector)
//std::cout << "Size of NVTXS vector: " << NVTXS.size() << std::endl;
// ... add similar prints for other vectors if needed

// 6. Verify other conditions: This is just an example. Adjust as necessary for your specific conditions.
//if (N <= 0 || IWLEN <= 0) {
//    std::cerr << "Error: Invalid values detected." << std::endl;
//    return; // or handle the error appropriately
//}
...
/////////////////////
    ...
    //  std::cout << "stage AMD 11" << std::endl;
      ...
      //std::cout << "stage AMD 12" << std::endl;
      ...
      //std::cout << "stage AMD 13" << std::endl;
    ...
    //std::cout << "stage AMD 14" << std::endl;

    // broadcast invp
    MPI_Bcast(&invp[0],N,type,0,comm);

    //std::cout << "stage AMD 15" << std::endl;
  ...
  void Ordering::METIS(const SparseMatrixGraph & g, MPI_Comm comm){
    ...
    if(iam==0){
      ...
      if(!isSameIdx){
        ...
        //for(Ptr i = 0; i<g.rowind.size();i++) std::cout << (idx_t)g.rowind[i] << ", ";
      ...
      if(!isSamePtr){
        ...
        //for(Ptr i = 0; i<g.colptr.size();i++) std::cout << (idx_t)g.colptr[i] << ", ";
      ...
      //for(int col=0; col<N;++col) std::cout << iperm[col] << ", ";
